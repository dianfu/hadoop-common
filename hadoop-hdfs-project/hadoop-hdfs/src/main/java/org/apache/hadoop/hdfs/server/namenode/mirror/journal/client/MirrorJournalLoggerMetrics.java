...
class MirrorJournalLoggerMetrics {
  final MetricsRegistry registry = new MetricsRegistry("NameNode");

  private volatile MirrorJournalLogger ch;
   ...
   */
  private static final Map<String, MirrorJournalLoggerMetrics> REGISTRY =
      Maps.newHashMap();

  private MirrorJournalLoggerMetrics(MirrorJournalLogger ch) {
    this.ch = ch;
    ...
    if (intervals != null) {
      ...
      for (int i = 0; i < writeRpcLatencyQuantiles.length; i++) {
  ...
  private void setChannel(MirrorJournalLogger ch) {
    assert ch.getRemoteAddress().equals(this.ch.getRemoteAddress());
    this.ch = ch;
  }

  static MirrorJournalLoggerMetrics create(MirrorJournalLogger ch) {
    String name = getName(ch);
    synchronized (REGISTRY) {
      MirrorJournalLoggerMetrics m = REGISTRY.get(name);
      if (m != null) {
        m.setChannel(ch);
      } else {
        m = new MirrorJournalLoggerMetrics(ch);
  ...
  private static String getName(MirrorJournalLogger ch) {
    URI addr = ch.getRemoteAddress();
    String addrStr = addr.getHost();
